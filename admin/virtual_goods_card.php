...
elseif ($_REQUEST['act'] == 'action')
{
   ...
   // $coded_old_card_sn = jiami($_POST['old_card_sn']);
    ...
    else
    {
        /* 更新数据 */
        $end_date = strtotime($_POST['end_dateYear'] . "-" . $_POST['end_dateMonth'] . "-" . $_POST['end_dateDay']);
        $sql = "UPDATE ".$ecs->table('virtual_goods_card')." SET card_sn='$coded_card_sn', card_password='$coded_card_password', end_date='$end_date' ".
...
function get_supplier_list(){
    $sql = "select supplier_id, supplier_name from ". $GLOBALS['ecs']->table('supplier');
