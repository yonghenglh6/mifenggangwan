...
function pre_build_attr_html($cat_id, $goods_id = 0, $bar_code = 0, $attr_list)
{
    $attr = get_attr_list($cat_id, $goods_id);
    foreach ($attr AS $key => $val) {
        foreach ($attr_list AS $key2 => $val2) {
            if($val[attr_id] == $val2[attr_id]){
                $attr[$key][attr_value] = $val2[attr_value];
            }
        }
    }
    $html = '<table width="100%" id="attrTable">';
...
function pre_goods_list()
{
    ...
    if ($result === false) {
        ...
        $where = " WHERE 1 = 1 ";
        $oeno =  empty($_REQUEST['oeno']) ? '' : trim($_REQUEST['oeno']);
        $filter['oeno'] = $oeno;
        /* 关键字 */
        if (!empty($oeno)) {
            $where .= " AND (s.oe LIKE '%" . mysql_like_quote($oeno) . "%') ";
        }

        $oename =  empty($_REQUEST['oename']) ? '' : trim($_REQUEST['oename']);
        $filter['oename'] = $oename;
        /* 关键字 */
        if (!empty($oename)) {
            $where .= " AND (s.`name` LIKE '%" . mysql_like_quote($oename) . "%') ";
        }

        $islink =  empty($_REQUEST['islink']) ? '' : trim($_REQUEST['islink']);
        if(empty($islink)){
            $islink = '0';
        }
        ...
        /* 关键字 */
        if ($islink == '0') {
            $where .= " AND (NOT EXISTS (SELECT 1 FROM ecs_goods g WHERE g.goods_oe = s.oe)) ";
        }else{
            $where .= " AND (EXISTS (SELECT 1 FROM ecs_goods g WHERE g.goods_oe = s.oe)) ";
        }
        /* 记录总数 */
        $sql = "SELECT COUNT(1) FROM  carpart s " . $where;
        ...
        $sql = "SELECT s.oe, s.`name`, gc.gc_name, " .
            " (CASE s.source WHEN 1 THEN '自有品牌件' WHEN 2 THEN '原厂件' WHEN 3 THEN '再制造件' WHEN 4 THEN '其他' WHEN 5 THEN '代理品牌件' WHEN 6 THEN '同质配套' WHEN 8 THEN '拆车件' END) source ".
            " FROM carpart s LEFT JOIN shopnc_goods_class gc ON s.cateid = gc.gc_id " . $where .
            " LIMIT " . $filter['start'] . ",$filter[page_size]";
