...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    if(intval($_REQUEST['supp'])>0){
    	...
    	// 入驻商商品列表不显示添加新商品
    	$action_link = ($_REQUEST['act'] == 'list') ? pre_add_link($code) : array('href' => 'goods.php?act=list', 'text' => $_LANG['01_goods_list']);
...
function pre_add_link($extension_code = '')
{
    $href = 'pre_goods.php?act=list';
    if (!empty($extension_code))
    {
        $href .= '&extension_code=' . $extension_code;
    }
